...
attribute vec3 myNormal, myVertex;
...
void main()
{
	...
	if (NdotL > 0.0) {
		...
		// If specular isn't set explicitly this throws stuff off.
      specular = vec4(0);//gl_FrontMaterial.specular * gl_LightSource[0].specular * pow(NdotHV,gl_FrontMaterial.shininess);
	}
	
	gl_FrontColor = globalAmbient + NdotL * diffuse + ambient + specular;

	gl_Position = ftransform();
   //gl_Position = gl_ModelViewProjectionMatrix * vec4(myVertex, 1.0);
